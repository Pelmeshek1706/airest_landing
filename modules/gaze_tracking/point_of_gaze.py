...
class PointOfGaze:
    ...
    def __init__(self, gaze_tracking, gaze_calibration, monitor, stabilize=False):
        """
        :param gaze_calibration: a GazeCalibration object, which holds a baseline iris_size,
        and the extreme values for gaze_ratios (for each edge of the computer screen)
        :param gaze_tracking: a modules object holding detected info for the eye and iris
        ...
        self.nb_same = 2  # Number of move fragments in the same direction in order to be an eye movement
        self.nb_interv = 0  # Number of intervening in diff direction within a smooth pursuit
        ...
        self.mx_intra_cluster_variation_const = 0.3  # Max variation within a cluster = 30 % of screen size
        self.mx_intra_cluster_dist = self.mx_intra_cluster_variation_const * monitor['width']
        self.current_cluster_x = deque(maxlen=self.cluster_mx_size)  # max_size, when queue is full
        self.current_cluster_y = deque(maxlen=self.cluster_mx_size)  # max_size, when queue is full
        self.candidate_cluster_x = deque(maxlen=self.cluster_mx_size)
        self.candidate_cluster_y = deque(maxlen=self.cluster_mx_size)
        self.move_cluster_x = deque(maxlen=self.cluster_mx_size)
        self.move_cluster_y = deque(maxlen=self.cluster_mx_size)
    ...
    def point_of_gaze(self):
        ...
        if self.gaze_tracking.pupils_located:
            ...
            # print(vr)
            est_x = (max(self.gaze_calibration.leftmost_hr - hr, 0) *
                     self.gaze_calibration.fsw * dist_factor) / \
                    (self.gaze_calibration.leftmost_hr - self.gaze_calibration.rightmost_hr)
            est_y = (max(vr - self.gaze_calibration.top_vr, 0) *
                     self.gaze_calibration.fsh * dist_factor) / \
