...
class PointOfGaze:
    ...
    def _init_cluster_parameters(self):
        ...
        self.norm_threshold = 0.7 # normalized distance threshold for cluster membership
        # maximum allowed variation within a cluster (20% of screen width)
        self.max_intra_cluster_dist_x = 0.2 * self.monitor['width']
        self.max_intra_cluster_dist_y = 0.1 * self.monitor['height']
    ...
    def _init_cluster_storage(self):
        ...
        self.num_clusters = 0 # FOR DEBUGGING
    ...
    def _update_current_cluster(self, x, y):
        ...
        if self._point_within_cluster(self.current_cluster_x, self.current_cluster_y, x, y):
            ...
            # Starting a new candidate cluster if none exists yet.
            if not self.candidate_cluster_x:
                self.num_clusters += 1
    ...
    def _within_cluster(self, cluster_points, x, y):
        ...
        return norm_diff <= self.norm_threshold  # use your chosen threshold
    ...
    def looking_straight_ahead(self, est_x, est_y):
        """
        ...
        """
        wmargin = self.frame_width * 0.3
        hmargin = self.frame_height * 0.5
        wmiddle = self.frame_width / 2
        hmiddle = self.frame_height / 2
